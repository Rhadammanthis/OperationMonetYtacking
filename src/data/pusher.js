...
class Pusher {
    ...
    pushExpensesData = async (data) => {
        ...
        let snapshot = await firebase.database().ref(`/${this.code}/expenses`).update(data).then((val) => console.log("In PUSHER", val))
        return snapshot
    }

    pushShoppingList = async (list) => {
        return await firebase.database().ref(`/${this.code}/shopping/`).set(list)
    }
    ...
    */
    updateShoppingList = async (index, active) => {
        return await firebase.database().ref(`/${this.code}/shopping/${index}/`).update(active)
    }
