...
class DataStore {
    ...
    addExpense = (day, category, amount) => {
        ...
        this.data[day].history[category].push({ amount: parseInt(amount), date: Date.now() })

        //Mark day on the calendar
        this.markedDates = { ...this.markedDates, [day]: { ...this.markedDates[day], marked: true } }
    ...
    selectDay = (day) => {
        ...
        this.markedDates = { ...this.markedDates, [day]: { ...this.markedDates[day], selected: true, disableTouchEvent: false } }

        //De-select previously selected day
        if (this.selectedDay !== null)
            this.markedDates = { ...this.markedDates, [this.selectedDay]: { ...this.markedDates[this.selectedDay], selected: false, disableTouchEvent: false } }
    ...
    getDaysTotal = () => { return this.daysTotal }
    ...
    updateDaysData = (day, category, value) => {
        ...
        if (value <= 0) {
            ...
            if (Object.keys(this.data[day].expenses).length === 0) {
                this.markedDates = { ...this.markedDates, [day]: { ...this.markedDates[day], marked: false } }
            }
        }
        else{
        ...
        this.setDaysExpenses(day)
        this.pusher.pushExpensesData(this.data).then((val) => console.log("Updated", val)).catch((reason) => console.log("Reason", reason))
    }
}
